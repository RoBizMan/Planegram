...
class UploadGramViewTest(TestCase):
    ...
    def test_upload_gram_view_get(self):
        ...
        self.assertTemplateUsed(response, 'upload/upload.html')  # Ensure the correct template is used
    ...
    def test_upload_gram_view_post_valid(self):
        ...
        self.assertTrue(Gram.objects.filter(caption='Test Caption').exists())  # Check if the Gram was created

    def test_upload_gram_view_post_invalid(self):
        form_data = {
            'date_photographed': datetime.date.today(),
            'caption': '',  # Invalid because caption is empty
            'image': '',  # Use an empty string instead of None
            'plane': self.aircraft.pk,
        }
        response = self.client.post(reverse('upload'), data=form_data)
        self.assertEqual(response.status_code, 200)  # Should return to the form with errors
        self.assertFormError(response, 'form', 'caption', 'This field is required.')  # Check for specific error