...
class GramViewsTest(TestCase):
    ...
    def test_report_gram_view(self):
        ...
        reporter = User.objects.create_user(username='reporter', password='reportpass')
    ...
    def test_report_gram_submission(self):
        ...
        # Submit the report
        response = self.client.post(reverse('report_gram', args=[self.gram.pk]), {'message': 'Inappropriate content'})
        
        # Check for a redirect after submission
        self.assertEqual(response.status_code, 302)  # Should redirect
        self.assertTrue(Report.objects.filter(gram=self.gram, user=reporter).exists())  # Check if report was created

    def test_gram_action_like(self):
        response = self.client.post(reverse('gram_action', args=[self.gram.pk]), {'action': 'like'})
        self.assertEqual(response.status_code, 302)  # Should redirect
        self.assertIn(self.user, self.gram.love.all())  # Check if user liked the gram

    def test_gram_action_delete(self):
        response = self.client.post(reverse('gram_action', args=[self.gram.pk]), {'action': 'delete'})
        self.assertEqual(response.status_code, 302)  # Should redirect
        self.assertFalse(Gram.objects.filter(pk=self.gram.pk).exists())  # Check if the gram was deleted