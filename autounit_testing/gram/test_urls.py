...
class GramUrlsTest(TestCase):
    ...
    def test_gram_detail_url_resolves(self):
        url = reverse('gram_detail', args=[self.gram.pk])  # Use the test gram's pk
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_gram_action_url_resolves(self):
        url = reverse('gram_action', args=[self.gram.pk])  # Use the test gram's pk
        response = self.client.post(url, {'action': 'like'})  # Simulate a like action
        self.assertEqual(response.status_code, 302)  # Should redirect

    def test_edit_gram_url_resolves(self):
        url = reverse('edit_gram', args=[self.gram.pk])  # Use the test gram's pk
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_report_gram_url_resolves(self):
        ...
        url = reverse('report_gram', args=[self.gram.pk])  # Use the test gram's pk
        response = self.client.get(url)
